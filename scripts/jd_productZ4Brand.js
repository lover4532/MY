...
!(async () => {
    ...
    let res = [];
    try{res = await getAuthorShareCode('https://raw.githubusercontent.com/star261/code/main/tw.json');}catch (e) {}
    if(!res){
        try{res = await getAuthorShareCode('https://cdn.jsdelivr.net/gh/star261/code@main/tw.json');}catch (e) {}
        if(!res){res = [];}
    }
    $.teamName = "left"
    if(res.length > 0){
        $.teamName = getRandomArrayElements(res,1);
    }
